 ...
 * Last Edit:   11/28/18
 *
 * Description: Connects to otp_dec and decrypts a message using modular addition.
 *  			It receives key and cipherfiles via a socket connection. After
 *				the message is decrypted, it sends the plaintext file back to the client.
 ***********************************************************************************/
...
int main(int argc, char *argv[]) {
	...
	socklen_t clientLength;     	// size of the client messages
	char buffer[BUFFER_SIZE],   	// buffer to hold sent messages
			message[BUFFER_SIZE],	// buffer to hold message
			auth[] = "d",			// authentication message
			invalid[] = "invalid";	// invalid connection message
	struct sockaddr_in serv_addr,  	// hold info for server connection
			client_addr;          	// hold info for client connection
	...
	if (DEBUG) { printf("Listening for connection.\n"); }

	while (1) {
		clientLength = sizeof(client_addr);

		newSocketFD = accept(socketFD, (struct sockaddr *) &client_addr, &clientLength); // accept incoming connection
		if (newSocketFD < 0) {
		...
		if (pid < 0) {
			print_error(1, "otp_dec_d error: Fork error.");
		}

		if (DEBUG) { printf("Now connected to client.\n"); }
		...
		if (pid == 0) {
			...
			if (DEBUG) { printf("Connection established with client.\n"); }
			...
			if (strcmp(buffer, "d") != 0) { // if authentication message is not found
			...
			if (DEBUG) { printf("Message received by server.\n"); }

			bzero(message, sizeof(message));

			strncpy(message, buffer, keyStart - buffer); 	// separate message and key

			decrypt_message(message, keyStart); 	// encrypt the message
